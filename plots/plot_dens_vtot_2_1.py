...
fname = [os.path.join(chk_dir,f'xx_zz_dens_time_{i:04}.npz') for i in (1, 95, 95) ]
...
def dens_(fontsize=18):
    ...
    for i in range(ny):
        for j in range(nx):
            ...
            if no_ij==0:
                ...
                axs[i,j].set_title(r'$\log~\rho~[g~cm^{-3}]$', fontsize=fontsize)
                cax = pl.axes([0.89, 0.66, 0.022, 0.2])
                cbobj = fig.colorbar(cf, cax=cax, format='%2.f', ticks=np.arange(-16., -7., 1))
                # cbobj = fig.colorbar(cf, cax=axs[i,j], format='%2.f', ticks=np.arange(-18., -7., 1))
            if no_ij==1:
                cf = plot_dens_ax(axs[i,j], no_ij, fontsize=fontsize)
                # axs[i,j].set_title(r'$\log~\rho~[g~cm^{-3}]$', fontsize=fontsize)
                ...
                # cbobj = fig.colorbar(cf, cax=axs[i,j], format='%2.f', ticks=np.arange(-18., -7., 1))
            if no_ij==2:
                cf = plot_vtot_ax(axs[i,j], no_ij, fontsize=fontsize)
                axs[i,j].set_title(r'$\log \left| \vec V \right| ~ \rm{[cm/s]}$', fontsize=fontsize)
                cax = pl.axes([0.89, 0.13, 0.022, 0.2])
                cbobj = fig.colorbar(cf, cax=cax, format='%2.1f', ticks=np.arange(6.5, 10, 1))
                # cbobj = fig.colorbar(cf, ax=axs[i,j], format='%2.1f', ticks=np.arange(2.9, 9.8, 1))
            if j==0:
    ...
    ofn = os.path.join(odir,f'dens_vtot_{xy}_{ny}_{nx}.png')
