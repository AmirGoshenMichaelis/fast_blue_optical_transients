...
for ang in $angle_list ; do
	ffmpeg -y -framerate $fs -pattern_type glob -i "${exp_name}/kappa_${kappa}/${ang}/kappa_${kappa}_los_${ang}_dens_*.png" -pix_fmt yuv420p -c:v libx264  ${exp_name}_k_${kappa}_a_${ang}_dens.mp4
	ffmpeg -y -framerate $fs -pattern_type glob -i "${exp_name}/kappa_${kappa}/${ang}/kappa_${kappa}_los_${ang}_temp_*.png" -pix_fmt yuv420p -c:v libx264  ${exp_name}_k_${kappa}_a_${ang}_temp.mp4
done
done
