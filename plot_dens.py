...
def already_exist(fn, odir, no):
    for src in ['xz']:
        ofn = os.path.join(odir,f'dens_{src}_{no:04}.png')
        if not os.path.exists(ofn):
            return False
    return True
###
def plot_dens_ax(param, ax, no=1, fontsize=14):
    ...
    return cf,Q
###
def dens_(param, plane='xz', fontsize=16):
    nx=1
    ny=1
    xy = plane
    figsize=(8,8)
    ...
    for i in range(ny):
        for j in range(nx):
            ...
            cf, Q = plot_dens_ax(param, ax=axs[i,j], no=i*nx+j, fontsize=fontsize)
            if i==ny-1:
    ...
    cbobj = fig.colorbar(cf, cax=cax, format='%2.f', ticks=np.arange(-18., -8. ,2))
