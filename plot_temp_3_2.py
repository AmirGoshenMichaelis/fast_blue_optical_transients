...
ffn = [None, None]
ffn[0] = [os.path.join(chk_dir,f'xx_zz_dens_time_{i:04}.npz') for i in (170,175,180,185,190,195) ]
...
###
def plot_temp_ax(ax, no, fontsize=16):
    ...
    decimate = 50
    cf = ax.contourf(X, Y, Z, np.linspace(3., 5, 512), cmap=cmap, extend='both')
    if no % 2 :
...
def temp_(fontsize=16):
    ...
    for i in range(ny):
        for j in range(nx):
            ...
            cf = plot_temp_ax(axs[i,j], i*nx+j, fontsize=fontsize)
            if i==ny-1:
    ...
    cbobj = fig.colorbar(cf, cax=cax, format='%2.f', ticks=np.arange(3, 6., 1))

    ofn = os.path.join(odir,f'temp_{xy}_{ny}_{nx}_{fname_set}.png')
